...
// gmm algorithms
#include "gmm_algorithms/u_s_gmm.hpp" // training model 1
#include "gmm_algorithms/s_gmm.hpp" // training model 2
...
public:
    ...
    em_time(0) {
        ...
        if (training_model == 1) {
            // u-S-GMM
            algo = std::make_unique<u_S_gmm<T>>(set, mean, H, nthreads, 1.0, R, verbose);
        } else if (training_model == 2) {
            // S-GMM
            algo = std::make_unique<S_gmm<T>>(set, mean, H, nthreads, 1.0, R, verbose);
        } else {
             throw std::logic_error("the training model does not exist: 1 = u-S-GMM | 2 = S-GMM");
        }
